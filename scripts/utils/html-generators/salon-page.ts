...
export function generateSalonPage(salon: BeautySalon): string {
  ...
  const metaDescription = `${salon.title} is a barbershop in ${cityName}, ${stateName}. ${salon.description?.substring(0, 150) || 'Visit us for quality haircuts, beard trims, and traditional shaves.'}`;
  ...
  if (salon.city && salon.city.salons && salon.city.salons.length > 1) {
    ...
    if (nearbySalons.length > 0) {
      ...
      <div class="mt-12 border-t border-gray-200 pt-12">
        <h2 class="text-2xl font-bold mb-6 text-blue-800">More Barbershops in ${cityName}</h2>
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
              <a href="${getSalonUrl(nearbySlug)}">
                ${nearbySalon.images && nearbySalon.images.length > 0 
                  ? `<img src="https://img.barbershopsnearby.com/your-r2-bucket-name${nearbySalon.images[0].path}" alt="${nearbySalon.title}" class="w-full h-40 object-cover">`
                  : `<div class="w-full h-40 bg-gradient-to-r from-blue-400 to-indigo-500 flex items-center justify-center text-white text-3xl">
              ...
              <div class="p-4">
                <a href="${getSalonUrl(nearbySlug)}" class="block">
                  <h3 class="font-bold text-lg text-blue-800 hover:text-blue-600">${nearbySalon.title}</h3>
                </a>
                <p class="text-gray-600 mt-1 text-sm">
                  <i class="fas fa-map-marker-alt text-blue-500 mr-1"></i> ${cityName}, ${stateName}
                </p>
                ${nearbySalon.telephone ? `
                <p class="mt-3">
                  <a href="tel:${nearbySalon.telephone}" class="inline-block bg-blue-600 text-white px-3 py-1 rounded text-sm hover:bg-blue-700">
  ...
  if (salon.latitude && salon.longitude) {
    ...
    mapEmbed = `
    <div class="mb-6">
      <h4 class="font-semibold mb-3 text-blue-800">Location</h4>
  ...
  <main class="container mx-auto px-4 py-6">
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
      ...
      <div class="relative h-40 md:h-56 bg-blue-800">
        ${salon.images && salon.images.length > 0 
          ? `<img src="https://img.barbershopsnearby.com/your-r2-bucket-name${salon.images[0].path}" alt="${salon.title}" class="w-full h-full object-cover opacity-70">`
          : `<div class="w-full h-full bg-gradient-to-r from-blue-800 to-indigo-800"></div>`
        }
        <div class="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 to-transparent">
          <div class="container mx-auto">
            <h1 class="text-2xl md:text-3xl font-bold text-white">${salon.title} - Barbershop in ${cityName}, ${stateName}</h1>
            <div class="flex flex-wrap items-center gap-2 mt-1 text-white/90 text-sm">
              ${salon.categories && salon.categories.length > 0 ? 
                `<div class="flex items-center">
                  <i class="fas fa-tag text-blue-300 mr-1"></i>
                  ${salon.categories.map(cat => cat.category).join(' • ')}
                </div>` : 
                `<div class="flex items-center">
                  <i class="fas fa-cut text-blue-300 mr-1"></i>
                  Barbershop
                </div>`
              }
              ...
              ${salon.telephone ? 
                `<div class="flex items-center ml-2">
                  <i class="fas fa-phone text-blue-300 mr-1"></i>
                  ${salon.telephone}
                </div>` : ''
              }
            </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 p-6">
          ...
          <div class="mb-8">
            ...
            <div class="bg-gray-50 p-5 rounded-lg border-l-4 border-blue-500">
              <p class="text-gray-700 leading-relaxed">
                ${salon.description || `${salon.title} offers professional haircuts, beard trims, and traditional shaves in ${cityName}, ${stateName}. Visit us for quality men's grooming services in a classic barbershop atmosphere.`}
              </p>
            </div>
          </div>
          ...
          <div class="mb-8">
            ...
            <div class="space-y-4">
              ${salon.reviews_data.map(review => `
                <div class="bg-gray-50 p-5 rounded-lg border-l-4 border-blue-300">
                  <div class="flex justify-between items-center mb-2">
                    <span class="font-bold text-blue-900">${review.author || 'Anonymous'}</span>
          ...
          <div class="mb-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
              ${salon.images.slice(1).map(image => 
                `<a href="https://img.barbershopsnearby.com/your-r2-bucket-name${image.path}" target="_blank" class="block h-40 overflow-hidden rounded-lg shadow-md">
                  <img src="https://img.barbershopsnearby.com/your-r2-bucket-name${image.path}" alt="${salon.title}" class="w-full h-full object-cover hover:opacity-90 transition-opacity">
                </a>`
              ).join('')}
            </div>
          </div>
        ...
        <div>
          <div class="bg-gray-50 rounded-lg p-6 shadow-md sticky top-20">
            <h3 class="text-xl font-bold mb-4 text-blue-800 border-b pb-2 border-gray-200">
            ...
            <div class="flex mb-4">
              ...
              <div>
                ...
                <a href="tel:${salon.telephone}" class="text-blue-600 hover:text-blue-800">${salon.telephone}</a>
              </div>
            </div>
            ` : ''}
            ...
            <div class="flex mb-4">
              ...
              <div>
                ...
                <a href="mailto:${salon.email}" class="text-blue-600 hover:text-blue-800">${salon.email}</a>
              </div>
            </div>
            ` : ''}
  ...
  return pageWrapper(`${salon.title} - Barbershop in ${cityName}, ${stateName}`, metaDescription, content);
}